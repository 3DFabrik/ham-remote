...
class UVK5Radio:
    ...
    def connect(self, port=None):
        ...
        try:
            ...
            # PTT blip to activate display stream (radio sends UI packets after PTT)
            logger.info("Activating display stream with PTT blip...")
            self.serial.rts = False  # PTT ON (inverted)
            time.sleep(1.0)
            self.serial.rts = True   # PTT OFF
            time.sleep(0.2)
    ...
    def _rx_loop(self):
        """Background thread: single serial reader, parses both 0xABCD and 0xB5."""
        import re
        self._ui_lines = {}
        buf = bytearray()
            ...
            try:
                ...
                while len(buf) > 0:
                    ...
                    if buf[0] == 0xB5:
                        ...
                        if ui_type in (0, 1, 2, 3) and data_len > 0:
                            try:
                                text = data.decode('ascii', errors='replace').rstrip('\x00')
                                if text:
                                    self._emit_display_cmd('text', {
                                        'type': ui_type, 'x': val1, 'y': val2,
                                        'font': val3, 'text': text
                                    })
                                    self._ui_lines[val2] = text
                                    try:
                                        freq_match = re.search(r'(\d{2,3}[.,]\d{3,6})', text)
                                        if freq_match:
                                            freq_str = freq_match.group(1).replace(',', '.')
                                            self._ui_text_freq = freq_str
                                            new_freq = float(freq_str)
                                            if 0.01 < new_freq < 1300 and abs(new_freq - self.current_freq) > 0.0001:
                            ...
                            self._emit_display_cmd('signal', {'v1': val1, 'v2': val2})
                        
                        continue
                    
                    # Check for 0xABCD binary packet
                    if len(buf) >= 2 and buf[0] == 0xAB and buf[1] == 0xCD:
                        if len(buf) < 4:
                            break
                        pkt_len = buf[2] | (buf[3] << 8)
                        ...
                        buf = buf[total_len:]
                        try:
                            result = QuanshengProtocol.parse_packet(packet_data)
                            if result:
                                cmd, params = result
    ...
    def _emit_display_cmd(self, cmd_type, data):
        """Emit a display command to all connected browsers via WebSocket."""
        try:
            socketio.emit('display_update', {'cmd': cmd_type, **data})
        except Exception:
            pass
